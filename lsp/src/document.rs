...
impl Document {
  ...
  pub fn get_relative_file_path(&self) -> Result<String> {
    let relative_path = self
      ...
      .map_err(|_| PresenceError::Config("File is not within the workspace root".to_string()))?;
  ...
  pub fn get_full_directory_name(&self) -> Result<String> {
    let parent_dir = self
      .path
      .parent()
      .ok_or_else(|| PresenceError::Config("Could not determine parent directory".to_string()))?;
  ...
  pub fn get_directory_name(&self) -> Result<String> {
    ...
    let dir_name = parent_dir
      .file_name()
      .ok_or_else(|| PresenceError::Config("Could not determine directory name".to_string()))?;
